...
namespace Microsoft.Azure.ServiceBusExplorer.Helpers
{
    ...
    internal static class CustomColorHelper
    {
        ...
        static Control GetParentForm(Control theControl)
        {
            ...
            {
                if (theControl.Parent.GetType() == typeof(MainForm))
                {
                    return theControl.Parent;
                }
                GetParentForm(theControl.Parent);                
            }
            if (theControl.GetType() == typeof(MainForm))
            {
                return theControl;
            }
            return null;
        ...
        public static Color CreateThemeColor1(Control ctrl)
        {
            var parentForm = GetParentForm(ctrl);
            if (parentForm != null && ((MainForm)parentForm).IsDarkThemed)
            {
                return CreateColorFromRGB(107, 107, 107);
            }
            return CreateColorFromRGB(215, 228, 242);
        }

        public static Color CreateThemeColor2(Control ctrl)
        {
            var parentForm = GetParentForm(ctrl);
            if (parentForm != null && ((MainForm) parentForm).IsDarkThemed)
            {
                return CreateColorFromRGB(99, 99, 99);
            }
            return CreateColorFromRGB(191, 205, 219);
        }

        public static Color CreateThemeColor3(Control ctrl)
        {
            var parentForm = GetParentForm(ctrl);
            if (parentForm != null && ((MainForm) parentForm).IsDarkThemed)
            {
                return CreateColorFromRGB(90, 90, 90);
            }
            return CreateColorFromRGB(153, 180, 209);
        }

        public static Color CreateThemeColor4(Control ctrl)
        {
            var parentForm = GetParentForm(ctrl);
            if (parentForm != null && ((MainForm)parentForm).IsDarkThemed)
            {
